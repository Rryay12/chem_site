...
class reaction:
    ...
    def __init__(self):
        self.molecule_info = {}
        self.k = 1  # Rate constant for the reaction
    ...
    def reaction(self, t, current_conc):
        ...
        epsilon = 1e-10  # To avoid division by zero or negative concentrations
        rate = self.k * np.prod([(current_conc[i] + epsilon) ** degree_list[i] for i in range(len(current_conc))])
        
        # Update concentrations based on the rate and degree (reaction order)
    ...
    def make_graph(self, t_span=None, num_points=100, link=""):
        ...
        plt.title('Unidirectional Reaction Simulation')
        plt.legend()
        plt.grid()
